...
public interface LeaveDetailsRepository extends JpaRepository<LeaveHistory, Integer> {
	...
	@Query(value = "SELECT * FROM leave_history l where MONTH(to_date) = :month AND YEAR(from_date) = :year AND status = 1",
			nativeQuery = true)
	List<LeaveHistory> findAllByMonth(@Param("month") Integer month, @Param("year") Integer year);
